...
class CharacterRepositoryImpl @Inject constructor(private val characterRemoteDataSource: CharacterRemoteDataSource) :
    ...
    override fun getAllCharacters(page: Int): Flow<DataState<CharacterResponse>> = characterRemoteDataSource.getAllCharacters(page = page)

    override fun getCharacter(characterId: Int): Flow<DataState<CharacterInfoResponse>> = characterRemoteDataSource.getCharacter(characterId = characterId)

    override fun getCharacter(url: String): Flow<DataState<CharacterInfoResponse>> = characterRemoteDataSource.getCharacter(url = url)}